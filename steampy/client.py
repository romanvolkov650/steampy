...
class SteamClient:
    ...
    def save_cookies(self):
        if not os.path.exists("cookies"):
            os.makedirs("cookies")
        with open(
            os.path.join("cookies", self.username + ".pkl"), "wb"
        ) as cookies_file:
    ...
    def load_cookies(self):
        if not os.path.exists(os.path.join("cookies", self.username + ".pkl")):
            return
        with open(
            os.path.join("cookies", self.username + ".pkl"), "rb"
        ) as cookies_file:
            try:
    ...
    def get_trade_url(self) -> str:
        url = f"{SteamUrl.COMMUNITY_URL}/my/tradeoffers/privacy"
        response = self._session.get(url)
        if trade_url := re.search(
            r'id="trade_offer_access_url" value="(.*?)"', response.text
        ):
            return trade_url.group(1)
        raise ValueError(f"Can't get trade url : {trade_url}")
    ...
    def fetch_api_key(self):
        url = f"{SteamUrl.COMMUNITY_URL}/dev/apikey/"
        response = self._session.get(
            url, params={"l": "english"}, allow_redirects=False
        )

        search = re.search(r"<p>Key: (?P<api_key>[0-9A-F]+)</p>", response.text)
        if not search:
            raise Exception("Failed to get Steam Web API key", response.text)

        self._api_key = search.group("api_key")
    ...
    def revoke_api_key(self):
        ...
        self._session.post(
            f"{SteamUrl.COMMUNITY_URL}/dev/revokekey", data=data, allow_redirects=False
        )
        self._api_key = None
    ...
    def register_new_api_key(self, domain: str = "localhost"):
        ...
        url = f"{SteamUrl.COMMUNITY_URL}/dev/requestkey/"
        response = self._session.post(url, data=data)
        print(response.status_code, response.text)
        rj = response.json()
        success = rj["success"]
        if success == 22 and rj["requires_confirmation"]:
            self._confirm_api_key_request(rj["request_id"])
            data.update({"request_id": rj["request_id"]})
            response = self._session.post(response.url, data=data)
            print(response.status_code, response.text)
            rj = response.json()
            success = rj["success"]

        if success != 1 or not rj["api_key"]:
            raise Exception("Failed to register new API key", success, response)
    ...
    def make_offer_with_url(
        ...
        headers = {
            ...
            "Referer": f"{SteamUrl.COMMUNITY_URL}/tradeoffer/new/?partner={partner_account_id}&token={token}",
            "Origin": SteamUrl.COMMUNITY_URL,
        }
